...
Realsense::Realsense(RealsenseConfig config): config(config) {
    rs2::config cfg;
    cfg.disable_all_streams();
    cfg.enable_stream(RS2_STREAM_DEPTH, 1280, 720, RS2_FORMAT_ANY, 15); // [fps]
    cfg.enable_stream(RS2_STREAM_COLOR, 1280, 720, RS2_FORMAT_ANY, 15); // [fps]

    pipe.start(cfg);

    takeImages();

    std::cout << "Realsense open." << std::endl;
}
...
cv::Mat Realsense::draw_pointcloud(float width, float height, bool draw_texture, texture& tex, rs2::points& points) {
    ...
    float alpha = 1.0 / (2 * config.pixel_size);
        ...
        for (int i = 0; i < points.size(); i++) {
            if (vertices[i].z) {
                if (!draw_texture) {
                    ...
                    float c = std::max(std::min((depth - config.max_depth) / (config.min_depth - config.max_depth), 1.0), 0.0);
                    glColor3f(c, c, c);
                }
...
std::pair<cv::Mat, cv::Mat> Realsense::takeImages() {
    ...
    try {
        ...
        std::cout << "error while waiting for realsense frames." << std::endl;
        pipe.stop();
        pipe.start();
        frames = pipe.wait_for_frames();
    }
