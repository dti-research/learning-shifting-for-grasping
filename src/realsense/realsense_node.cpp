...
#include <learning_shifting/OrthographicImage.h>
#include <learning_shifting/GetOrthographicImages.h>
...
class RealsenseNode {
  ...
  learning_shifting::OrthographicImage convert(const cv::Mat& image, RealsenseConfig config, const std::string& encoding, const std::string& suffix) {
    const sensor_msgs::ImagePtr image_ptr = cv_bridge::CvImage(std_msgs::Header(), encoding, image).toImageMsg();
    learning_shifting::OrthographicImage result;
  ...
  bool getImages(learning_shifting::GetOrthographicImages::Request &req, learning_shifting::GetOrthographicImages::Response &res) {
